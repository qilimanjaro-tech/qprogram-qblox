...
The bundle a platform puts in the bus slots of every qblox-driven bus: the tokens those buses support, the numeric limits that go with them, and the predicates that flag combinations the tokens alone would let through. See [Capabilities and profiles](../guide/capabilities.md) for how a platform assembles it into a `PlatformCapabilities`.
